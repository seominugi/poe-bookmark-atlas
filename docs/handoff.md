...
- **라이브 확인 완료(2026-07-27)**: 조건 묶음 등록·얹기 ✓ / 리그 이관 ✓ (미완료 6번 해소)
- **라이브 확인 대기 1건**: **Alt+F 검색칸 삭제** — 실제 키보드 입력이 있어야 검증된다(`keydown` 가드 + `navigator.userActivation` 판별이 핵심이라 합성 이벤트로는 무의미). 사용자 직접 확인 필요
