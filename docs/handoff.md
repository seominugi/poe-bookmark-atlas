...
**0.6.5 릴리즈 게시 완료 · 스토어 제출 대기 (2026-08-06)** — 버전 범프(`42e349d`) → `develop` → `main` fast-forward(`6836a23..42e349d`, 9커밋) → **`v0.6.5` 태그 즉시 publish(現 `Latest`)**. `deploy/poe-bookmark-atlas-0.6.5.zip`(31파일·801KB, manifest 루트 확인) 생성. **남은 것: 사용자가 스토어 심사 제출.**
