...
| `priceSnapshot` | content script | 렌더된 결과 listings 가격 수집 → 환율 환산 → 동적 하위절사 → P25 (`compute_sellable_price` 미러) |
...
1. 렌더된 결과 listings에서 `{amount, currency}` 수집. 가격 오름차순 정렬이 아니면 환율 환산 비교로 클라이언트에서 정렬.
...
5. **표시**: `value ≥ 1 div` → `≈ X.X div`; `< 1 div` → 가독성 위해 하위 단위(예: `≈ N ex`).

**폴백**: 환율 조회 실패 시 → 디바인 환산을 포기하고, **최빈 통화** listings만으로 동일 절차(동적 절사 → P25)를 적용해 **그 통화 그대로** 표시(`≈ 150 chaos`). 외부 의존 없이도 의미 유지.
...
// SearchRecord — chrome.storage.local에 평면 배열로 저장
{
  ...
  "snapshot": {                    // (선택) 가격 스냅샷 — compute_sellable_price 미러
    ...
    "unit": "divine",              // 표시 단위 (폴백 시 "chaos" 등 raw 통화)
...
| 가격 | `compute_sellable_price` 미러(동적 하위절사 `max(1,⌊n×0.1⌋)` → P25), 리그별 환율로 디바인 환산, 저장 시점 스냅샷 + 수동 갱신 |
