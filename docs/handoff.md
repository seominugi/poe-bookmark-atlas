...
- 테스트 **207/207**(신규 42: `tradeSearch` 17 + `store` 리그이관 6 + `leagueMigration.dom` 19) · 빌드 통과
...
6-b. **[확인 대기] URL의 리그 세그먼트만 바꿔도 같은 조건으로 검색되는가** — 사용자 관찰(2026-07-22): 같은 검색 해시가 `/trade/search/Hardcore/EBo8vB8LC5`·`/trade/search/Standard/EBo8vB8LC5` 처럼 리그만 바꿔 열린다. **사실이라면 이관을 URL 치환만으로 할 수 있고, 저장된 조건(query)이 없는 구 북마크까지 전부 복구된다**(현재 구현의 유일한 사각지대). 검증법: 실제로 리그만 바꾼 URL이 같은 필터로 결과를 내는지 + 오래된 해시도 살아있는지. 살아있는 해시면 URL 치환, 죽었으면 저장된 query로 POST — 2단 폴백이 이상적. 개발 환경은 로그인 세션이 없어 확인 불가
