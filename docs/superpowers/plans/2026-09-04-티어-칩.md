...
> ⚠ **아래 스크립트 본문은 구현 중에 세 번 고쳐졌다. 그대로 복사하지 말 것** —
> 정본은 `scripts/build-tier-table.mjs` 다. 무엇이 왜 바뀌었는지는 설계 문서 §5.2a 와
> 커밋 `4d3024f`·후속 수정 커밋에 남아 있다. 특히 아래 코드에는 **값 오염 버그**가 있다:
> `mod.valueRanges` 를 문장 단위로 자르지 않아, 하이브리드 모드에서 한 문장짜리 능력치에
> 옆 문장 값이 따라붙는다(25건 발견). 실제 스크립트는 `rangesByLine()` 으로 자른다.
...
매칭률이 90% 아래로 나오면 멈추고 보고한다 — 거래소 문구가 바뀐 것이다.
