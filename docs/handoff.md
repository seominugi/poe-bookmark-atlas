...
**0.5.0 배포 준비 (2026-07-27)** — 조건 묶음·조건 칩 클릭·일괄 폴더 이동·환산 칩 확대를 담아 **0.5.0으로 범프**(manifest·package)하고 `deploy/poe-bookmark-atlas-0.5.0.zip` 생성 완료(29파일·725KB, 테스트 287/287·빌드 통과). 이동 모달·저장 다이얼로그 **라이브 검증 완료**.
**남은 것: ① `develop` → `main` 머지(보호 브랜치라 사용자 확인 필요) ② GitHub 릴리즈 `v0.5.0` draft 생성 ③ 사용자가 스토어에 zip 제출 → 심사 통과 후 draft publish.**
...
- 배포: **0.4.0 스토어 게시 중**(리그 이관·투어 데모·내 리그 설정) / **0.5.0 범프·zip 완료**(조건 묶음·조건 칩 클릭·일괄 폴더 이동·환산 칩 확대) — main 머지·릴리즈 draft·스토어 제출 대기
