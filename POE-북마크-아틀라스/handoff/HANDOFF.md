...
## ⛔ 이식 규칙 (필독 — Claude Code에게 그대로 전달)

> **이 작업은 "재설계"가 아니라 "그대로 이식"이다.** 아래를 반드시 지킬 것:
>
> 1. **색·값을 새로 만들지 말 것.** 색상은 `panel.css`의 `:host { --ba-* }` 토큰과
>    프로토타입 인라인 값만 사용한다. "디자인 시스템", "슬레이트", "에메랄드" 등
>    임의 팔레트로 치환 금지. (과거 구현에서 패널을 슬레이트+에메랄드로 바꾼 사례 = 잘못)
> 2. **테마 = 자수정 글래스(보라).** 패널 배경 `rgba(24,21,42,.97)`, 테두리는
>    **상단 초록 → 하단 보라 그라데이션**(이중 배경 트릭, `panel.css`의 `.ba-root` 참고 — 정적이라 성능 무관).
>    단색 슬레이트 배경이나 초록 단색 chrome은 금지.
> 3. `handoff/panel.css`를 **그대로 복사**해 `src/content/panel/panel.css`로 쓴다.
>    값을 "개선"하거나 "정리"하지 말 것. 클래스명도 100% 유지.
> 4. 기능 추가(§2~11)는 **마크업만** 추가하고 스타일은 panel.css 기존 규칙을 재사용한다.
> 5. **핸들 위치**: `.ba-handle { right: 398px }` — 패널 좌측 모서리(`right:14 + width:384 = 398`)와
>    정확히 일치해야 틈이 안 생긴다. 402px 등 다른 값 금지.
> 6. 확신이 안 서면 프로토타입(`북마크 아틀라스.dc.html` / `확장 팝업.dc.html`)의
>    인라인 style 값을 그대로 읽어 쓴다. 추측 금지.
...
```
handoff/
├─ HANDOFF.md                       ← 이 문서 (이식 규칙 + 기능 명세)
├─ panel.css                        ← 패널 드롭인 CSS (자수정 글래스 테마 + v1.0 클래스)
├─ 디자인 프로토타입 (미리보기).html   ← ⭐ 패널 픽셀 기준 렌더 (현재 디자인 그대로)
├─ 확장 팝업 (미리보기).html        ← ⭐ 팝업 픽셀 기준 렌더
└─ icons/
   ├─ mascot-analyst.webp           ← 아이템 시세 마스코트(애널리스트)
   └─ mascot-researcher.webp        ← 시장 동향 마스코트(리서처)
```

> **픽셀 기준(SSOT) = 미리보기 렌더 2개.** 색·간격·구조가 애매하면
> `panel.css`가 아닌 이 렌더를 열어 그대로 맞춘다. 프로토타입(`북마크 아틀라스.dc.html`)과
> 동일한 결과다.
>
> **팝업(`src/popup/`)은 이미 디자인과 일치** — 새로 그리지 말고 그대로 둔다. (참고 렌더 = 위 팝업 미리보기)
...
## 12. v1.0 클래스 명세 (panel.css 기준 — 마크업은 이 클래스를 emit)

| UI | 클래스 | 비고 |
|---|---|---|
| 빠른 검색 행 | `.ba-search-row` > `.ba-search` (input + `.clear`) + `.ba-seg` | |
| 정렬/밀도 분절 토글 | `.ba-seg` > `span`, 활성 `.active` | 순서·최근·이름 / 여유·조밀 공용 |
| 리그·갱신 통합 배지 | `.ba-attn` (앰버, 클릭=갱신·재검색) | 흐림 행 `.ba-row.ba-attn-dim` |
| 조건 칩 / 더보기 | `.ba-chips` > `.ba-chip`, `.ba-chip-more` | |
| 이름=다시열기 칩 | `.ba-open` (북마크는 이 칩만 재검색) | |
| 가격 알약 | `.ba-price-pill` (앰버, divine/exalted img) | |
| 카드 하단 액션 | `.ba-rowfoot` > `.time` + `.acts` > `.ba-act.copy/.over/.rename/.del` (26px) | |
| 폴더 색 띠 | `.ba-folder-head` `border-left:3px solid <folder.color>` | 5색 팔레트 §11-4 |
| 밀도 조밀 | `.ba-list.dense` | 카드 패딩·이름 축소 |
| 대용량 | `.ba-row { content-visibility:auto }` | |

> 위 표의 클래스 스타일은 `panel.css` 하단 "v1.0 신규 UI" 섹션에 모두 정의돼 있다.
> 카드 내부의 정확한 배치·간격이 헷갈리면 `디자인 프로토타입 (미리보기).html`을 열어 그대로 맞춘다.
