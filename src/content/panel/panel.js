...
최소 너비 아래로는 줄지 않아요"></div>
  ...
  const TOUR = [
    ...
    { sel: '.ba-open', since: '0.8.0', title: '한 번에 다시 열기', body: '북마크 이름을 클릭하면 그 검색을 거래소에서 그대로 다시 엽니다. 복잡한 조건을 다시 짤 필요가 없어요. 어디에 열지는 설정에서 정하고(현재 탭 / 새 탭), <b>Ctrl 클릭</b>은 언제나 그 반대로 엽니다.' },
    // 라이브 전용 스텝 — ⋯ 안의 '.ba-live' 를 직접 가리킬 수 없다(팝오버가 hidden 이라 크기 0).
    // 항상 보이는 이름 칩을 가리키고, 거기서 실제로 되는 Shift 클릭을 앞세운다.
    { sel: '.ba-open', since: '0.8.0', title: '새 매물을 기다리지 말고 — 라이브로 열기', body: '북마크 이름을 <b>Shift 클릭</b>(또는 ⋯ → "라이브로 열기")하면, 거래소의 <b>라이브 검색</b>이 켜진 채로 새 탭에서 열려요. 조건에 맞는 매물이 새로 올라오면 그 탭에 바로 나타납니다 — 새로고침할 필요가 없어요. 지난 리그 북마크는 지금 리그로 되살린 뒤 켭니다.' },
