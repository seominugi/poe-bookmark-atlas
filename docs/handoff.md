...
0.2.0 스토어 심사 대기 중(제출일 기준 최신 확인 필요). 피드백 3건(작업1·2·3) 모두 기능 구현 완료. **사용자 방침(2026-07-04): "번역 100% 완벽 안 된 상태 인정하고, 미진한 부분은 추후 보완"** — Shift+클릭 수동 제보 기능이 그 보완 파이프라인. 이후 가이드 투어 화살표·키보드 네비·리그 노출 라운드까지 마치고 이번 커밋으로 origin push 완료 — 0.3.0 후보 상태(버전 범프·스토어 제출은 미결정, 미완료 6·7번 참조).
...
- 브랜치: `main` — 이 라운드 커밋·push 완료 예정(아래 표기는 커밋 시점 기준, 다음 세션에서 `git status`로 재확인 권장)
- 테스트: **165/165**(vitest, jsdom 포함) · 빌드 통과
- 배포: **0.2.0 스토어 심사 중**(제출일 기준 최신 확인 필요) / 이번 라운드 전체는 0.3.0 후보 — **버전 범프·스토어 제출은 아직 안 함**, 0.2.0 심사 상태 확인 후 사용자와 함께 결정
