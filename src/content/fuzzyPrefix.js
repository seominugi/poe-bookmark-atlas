...
// 능력치(스탯) 필터 검색칸 + 아이템 검색칸에 포커스가 가면 비어 있을 때 "~"를 자동 입력해
...
// 스탯 필터("+ 능력치 필터 추가") · 아이템 검색("아이템 검색…") 입력칸 식별 (그룹 추가는 제외)
function isFuzzyTarget(el) {
...
export function initFuzzyPrefix() {
  document.addEventListener(
    ...
    (e) => {
      const el = e.target
      if (!isFuzzyTarget(el) || el.value !== '') return
