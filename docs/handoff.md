...
**0.6.1 릴리즈 준비 완료 · 스토어 제출 대기 (2026-08-01)** — 조건 묶음 드래그 재배치·삭제, 카오스↔신성한 양방향 환산 칩, 하드코어 리그명 한글화, 변형 아이템 북마크명 버그 수정을 담아 **v0.6.0 태그 게시 완료**(`cd5dbc4`). 이후 사용자 실사용 제보로 드래그 사용성을 정비하고 칩 줄 접기를 더해 **0.6.1** 범프.
**남은 것: ① `develop` → `main` FF 머지·푸시(보호 브랜치 — 사용자 확인 후) ② `v0.6.1` 태그 게시 ③ 사용자가 `deploy/poe-bookmark-atlas-0.6.1.zip` 스토어 제출.**
⚠ **0.6.0 zip은 스토어에 제출하지 않았다**(사용자 확인, 2026-08-01) — 0.6.1이 스토어로 가는 첫 버전이라 0.6.0 zip은 폐기한다. 태그 v0.6.0은 GitHub에만 남는다.
...
**남은 것: 사용자가 스토어에 zip 제출 → 심사 결과 회신.**
...
7. **[사용자 대기] 0.5.0 스토어 제출** — `deploy/poe-bookmark-atlas-0.5.0.zip`(로컬만 존재, gitignore). 태그 `v0.5.0`은 이미 게시됨. 심사 결과 회신되면 이 항목 종료
...
- 배포: **0.5.0 릴리즈 게시 완료**(태그 `v0.5.0`, `13dd975`, 現 `Latest`) — **스토어 제출은 사용자 대기**. 0.4.0은 스토어 게시 중
