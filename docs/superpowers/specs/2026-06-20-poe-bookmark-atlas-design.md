...
> [!NOTE]
> 이 문서는 2026-06-20 당시의 설계 기록입니다. 당시 MIT 공개를 전제로 한
> 라이선스 설명은 태그 `v0.14.1` 이하에만 해당합니다. 현재 조건은 저장소
> 루트의 [`LICENSE`](../../../LICENSE)를 따릅니다.

- **작성일**: 2026-06-20 · **갱신**: 2026-06-21 (v1.1)
- **상태**: v1.1 구현 완료 (로컬 검증 완료, 웹스토어 배포 전) · 단위 테스트 40
- **레포**: `poe-bookmark-atlas` · **브랜드명**: 북마크 아틀라스
- **당시 라이선스**: MIT (태그 `v0.14.1` 이하)
...
### 포함 (v1)
- 검색 **북마크** (이름 저장, 영구) · 검색 **히스토리** (자동 기록, 최근 50개)
- 히스토리 → 북마크 **승격** (☆, 이름 변경 가능)
- **가격 스냅샷** (저장 시점 sellable, div·ex)
- 우측 **도킹 패널** (Shadow DOM·글래스), **핸들** 토글, **탭** UI (북마크 / 히스토리)
- seominugi.com **경제 페이지 링크**

### 포함 (v1.1 — 2026-06-21 추가)
- **폴더** 정리 (생성·이름변경·삭제, 폴더별 ➕ 저장, 미분류 그룹)
- **드래그앤드롭** 북마크 순서 변경 + 폴더 간 이동 (순수 HTML5)
- 북마크 **🔄 "최근 검색으로 덮어쓰기"** (이름·폴더·순서 유지)
- **POE1 / POE2 게임별 분리** (현재 페이지 게임의 북마크·히스토리·폴더만 표시)
- **능력치 필터 "~" 퍼지 기본 입력** (스탯 검색칸 포커스 시 자동 `~`)
- **패널 내 인라인 이름 입력**(네이티브 prompt 대체) · **글래스모피즘 디자인**(seominugi-com 채용)

### 제외 (v2 후보)
- 리그 변경 시 저장 검색 **자동 변환**
- 클라우드 **동기화 / 공유** · 태그(폴더로 대체)
...
// SearchRecord — chrome.storage.local에 평면 배열로 저장
{
  ...
  "folderId": "f_abc" | null,      // (북마크) 소속 폴더, null=미분류 — v1.1
  "order": 3,                       // (북마크) 정렬 인덱스(드래그앤드롭) — v1.1
  "createdAt": 1718800000000,
  "updatedAt": 1718800000000
}

// 폴더 — chrome.storage.local의 별도 'folders' 배열 (v1.1)
{ "id": "f_abc", "name": "갑옷", "game": "poe2" | "poe1" | null }
```
...
- `chrome.storage.local`: `records` (SearchRecord[]) + `folders` (Folder[]) + `uiCollapsed` (패널 접힘 상태).
- 히스토리·북마크는 `kind`로 구분(승격은 `kind` 변경). **조회는 현재 게임(poe1/poe2)으로 스코프**(§16).
...
- **단위 테스트 40개**(순수 로직): `formatPrice`·`priceSnapshot`(절사·환산·P25)·`currencyRates`·`searchParser`·`statMap`·`store`(CRUD·폴더·순서·덮어쓰기·게임 스코프). 실제 검색 payload를 **픽스처**로 사용.
- **통합/수동**: unpacked(`dist/`) 로드 후 `poe.kakaogames.com` 거래 페이지에서 캡처·렌더·재검색·DnD·게임분리 검증(로그인 필요로 E2E 자동화는 제한적).
...
- `"~"` 퍼지 입력은 **placeholder "능력치 필터" 기준 DOM 의존** — 사이트 업데이트 시 셀렉터 점검 필요(설정 토글 추가 검토).
- 좁은 화면 기본 접힘 **임계값 1700px**·DnD `order` 부동소수 — 필요 시 재인덱싱/조정.
- v2 후보: 리그 자동 변환, 클라우드 동기화/공유, 글로벌 `pathofexile.com` 지원.
- (해결됨) 환율 베이스 호스트 `seominugi.com`·realmName 언더스코어·폴더/정렬·게임 분리.

---

## 16. v1.1 추가 기능 상세 (2026-06-21)

대부분 **우리 패널·우리 데이터 안**의 기능이라 페이지 DOM 비의존(견고). "~"만 예외(페이지 입력 조작).

- **폴더**: `folders` 컬렉션(`{id, name, game}`). 북마크는 `folderId`로 소속, `null`=미분류. 폴더 헤더에 ➕(현재 검색 저장)·✎(이름)·🗑(삭제 시 소속 북마크는 미분류로). 미분류 그룹은 항상 표시(폴더 밖 복귀 드롭 타깃).
- **드래그앤드롭**: 북마크 행 `draggable`, 순수 HTML5. 드롭 시 `order`를 이웃 중간값으로, 폴더 바디로 드롭하면 `folderId` 변경. 외부 라이브러리 0.
- **덮어쓰기(🔄)**: `overwriteBookmark(id, source)` — 북마크의 검색/스냅샷만 최근 히스토리로 교체, `name·folderId·order` 유지.
- **게임 분리**: `listByKind(kind, game)`·`listFolders(game)`·`addFolder(name, game)`가 현재 게임으로 필터. 패널 타이틀에 `· POE1/POE2`. 레거시(게임 미지정) 폴더는 양쪽 표시.
- **"~" 퍼지**(`fuzzyPrefix.js`): placeholder가 "능력치 필터"인 입력칸 포커스+비어있을 때 `~` 자동 입력(네이티브 setter + `input` 이벤트로 React/Vue 제어입력 대응).
- **show/hide**: 우측 **핸들** 클릭으로 접기/펼치기(`uiCollapsed` 저장). **좁은 화면(<1700px) 기본 접힘**으로 검색 영역 겹침 방지. 핸들·패널 모두 **글래스 재질**(seominugi-com `premium-glass-card` 채용).
- **인라인 이름 입력**: 네이티브 `prompt` 대신 패널 내 입력바 + 토스트.
- **모듈**: `panel/renderList.js`(폴더 그룹·DnD·행 액션), `panel/panel.js`(셸·인라인입력·토스트·핸들), `content/fuzzyPrefix.js` 추가. (`panel/rowActions.js`는 renderList에 통합되어 제거.)
...
| 저장 | `chrome.storage.local`: records + folders + uiCollapsed |
| 폴더(v1.1) | 게임별 폴더 + DnD 정렬/이동 + 폴더별 ➕ 저장 + 🔄 덮어쓰기 |
| 게임 분리(v1.1) | POE1/POE2 북마크·히스토리·폴더 분리 표시 |
| "~"(v1.1) | 능력치 필터 포커스 시 퍼지 자동입력(DOM 의존) |
| 배포 | 크롬 웹스토어 · MIT 오픈소스 |
